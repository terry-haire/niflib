...
void NiGeometry::FixLinks( const map<unsigned int,NiObjectRef> & objects, list<unsigned int> & link_stack, list<NiObjectRef> & missing_link_stack, const NifInfo & info ) {
	...
	if ( ( info.version >= 0x14020007 ) && ( info.userVersion == 12 ) ) {
		propertyLink1 = FixLink<>( objects, link_stack, missing_link_stack, info );
		propertyLink2 = FixLink<>( objects, link_stack, missing_link_stack, info );
	};
...
Ref NiGeometry::GetPropertyLink1() const {
	return propertyLink1;
}

void NiGeometry::SetPropertyLink1( Ref value ) {
	propertyLink1 = value;
}

Ref NiGeometry::GetPropertyLink2() const {
	return propertyLink2;
}

void NiGeometry::SetPropertyLink2( Ref value ) {
	propertyLink2 = value;
}
